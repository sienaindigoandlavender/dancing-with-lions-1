    ...
    <main className="min-h-screen bg-[#0a0a0a] text-[#e5e5e5]" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
      ...
      <section data-sid="institutions" className="py-24 px-6">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('institutions') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-10 leading-relaxed">One still teaches. One stands as monument. Both shaped the intellectual history of the Islamic world.</p>

          <div className="flex gap-2 mb-8">
            {INSTITUTIONS.map((inst, i) => (
              <button key={i} onClick={() => setActiveInstitution(i)}
                className={`flex-1 text-left p-4 border transition-all duration-300 ${activeInstitution === i ? 'border-[#B08D57]/60 bg-[#B08D57]/5' : 'border-neutral-800 hover:border-neutral-700'}`}>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="flex-1">
                ...
                <div className="space-y-3 text-sm text-neutral-400 leading-relaxed">
                  ...
                  <p className="text-neutral-300 pt-2 border-t border-neutral-800/50">{INSTITUTIONS[activeInstitution].detail}</p>
                </div>
              </div>
              <div className="lg:w-72 flex-shrink-0">
                <div className="border border-neutral-800 p-4 bg-[#B08D57]/[0.03]">
      ...
      {/* ── SCHOLARS ── */}
      <section data-sid="scholars" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('scholars') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-10 leading-relaxed">A Jewish philosopher. A future Pope. The father of sociology. The mapmaker who guided Columbus. They all passed through Fez.</p>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-8">
            {SCHOLARS.map((s, i) => (
              <button key={i} onClick={() => setActiveScholar(i)}
                className={`text-left p-3 border transition-all duration-300 ${activeScholar === i ? 'border-[#B08D57]/60 bg-[#B08D57]/5' : 'border-neutral-800 hover:border-neutral-700'}`}>
          ...
          <div className="border border-neutral-800 p-6 md:p-8">
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex-1">
                ...
                <p className="text-sm text-neutral-400 leading-relaxed">{SCHOLARS[activeScholar].contribution}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ── THE LIBRARY ── */}
      <section data-sid="library" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('library') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="space-y-3">
            {visibleLibrary.map((item, i) => (
              <div key={i} className="flex items-start gap-4 border border-neutral-800 p-4 hover:border-neutral-700 transition-colors">
      ...
      {/* ── THE DEBATE ── */}
      <section data-sid="debate" className="py-24 px-6 border-t border-neutral-900">
                ...
                {expandedDebate === i && (
                  <div className="px-4 md:px-5 pb-5 space-y-4">
                    <div className="border-l-2 pl-4" style={{ borderColor: '#22C55E' }}>
                      ...
                      <p className="text-sm text-neutral-400 leading-relaxed">{d.forMorocco}</p>
                    </div>
                    <div className="border-l-2 border-neutral-700 pl-4">
                      ...
                      <p className="text-sm text-neutral-400 leading-relaxed">{d.counterArgument}</p>
                    </div>
                  </div>
                )}
              </div>
      ...
      {/* ── COMPARISON TABLE ── */}
      <section data-sid="compare" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('compare') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="space-y-2">
            {COMPARISONS.map((c, i) => (
              <div key={i} className={`flex flex-col md:flex-row md:items-center gap-2 md:gap-6 border p-4 transition-colors ${i === 0 ? 'border-[#B08D57]/40 bg-[#B08D57]/[0.03]' : 'border-neutral-800 hover:border-neutral-700'}`}>
                ...
                <div className="flex-1">
                  <span className={`text-sm ${i === 0 ? 'text-neutral-200' : 'text-neutral-400'}`}>{c.institution}</span>
      ...
      <section data-sid="timeline" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('timeline') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-8 leading-relaxed">From Fatima&rsquo;s inheritance to Aziza&rsquo;s restoration.</p>

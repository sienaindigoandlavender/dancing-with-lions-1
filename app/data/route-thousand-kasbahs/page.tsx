    ...
    <main className="min-h-screen bg-[#0a0a0a] text-[#e5e5e5]" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
      ...
      <section data-sid="kasbahs" className="py-24 px-6">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('kasbahs') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-12 leading-relaxed">Each one a family&apos;s claim staked in earth. Built where trade routes crossed rivers, where passes funnelled caravans, where power needed a stage.</p>

          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-8">
            {KASBAHS.map((k, i) => (
              <button key={i} onClick={() => setActiveKasbah(i)}
                className={`text-left p-4 border transition-all duration-300 ${activeKasbah === i ? 'border-[#C2703E]/60 bg-[#C2703E]/5' : 'border-neutral-800 hover:border-neutral-700'}`}>
            ...
            <div className="flex flex-col md:flex-row md:items-start gap-6">
              <div className="flex-1">
                ...
                <div className="space-y-3 text-sm text-neutral-400 leading-relaxed">
                  ...
                  <p className="text-neutral-300">{KASBAHS[activeKasbah].detail}</p>
                </div>
              </div>
              <div className="md:w-64 flex-shrink-0">
                <div className="border border-neutral-800 p-4 bg-[#C2703E]/[0.03]">
      ...
      {/* ── ARCHITECTURE ── */}
      <section data-sid="arch" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('arch') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-12 leading-relaxed">Every element is a dialogue with the environment. Local materials, flat roofs for drying crops, thick walls for insulation, minimal openings for protection. Sustainable long before green design became a trend.</p>
              ...
              <div key={i} className="border border-neutral-800 overflow-hidden">
                ...
                {expandedArch === i && (
                  <div className="px-4 md:px-5 pb-4 md:pb-5">
                    ...
                    <p className="text-sm text-neutral-400 leading-relaxed max-w-3xl">{a.detail}</p>
                  </div>
                )}
              </div>
            ))}
      ...
      {/* ── THE ROUTE ── */}
      <section data-sid="route" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('route') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-12 leading-relaxed">From the highest paved pass in North Africa down to the first dunes. The route descends through valleys, gorges, and oases — each stop a world.</p>
            ...
            <div className="space-y-1">
              {ROUTE_STOPS.map((stop, i) => (
                <div key={i}
                  ...
                  {/* Dot */}
                  <div className={`absolute left-[11px] md:left-[19px] top-[18px] md:top-[22px] w-2.5 h-2.5 rounded-full border-2 transition-colors ${activeStop === i ? 'border-[#C2703E] bg-[#C2703E]' : 'border-neutral-600 bg-[#0a0a0a]'}`} />
                  ...
                  {activeStop === i && (
                    <p className="text-sm text-neutral-400 mt-2 leading-relaxed max-w-xl">{stop.highlight}</p>
                  )}
                </div>
              ))}
            </div>
      ...
      {/* ── HOLLYWOOD ── */}
      <section data-sid="films" className="py-24 px-6 border-t border-neutral-900">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {FILMS.map((f, i) => (
              <div key={i} className="border border-neutral-800 p-4 hover:border-neutral-700 transition-colors group">
                ...
                <div className="text-xs text-neutral-400 mt-1">{f.role}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── GLAOUI ── */}
      <section data-sid="glaoui" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('glaoui') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              ...
              <div key={i} className="border border-neutral-800 p-5 md:p-6">
      ...
      <section data-sid="timeline" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('timeline') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-8 leading-relaxed">Twenty-six centuries of earth, power, and cinema.</p>

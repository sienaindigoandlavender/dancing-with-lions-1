    ...
    <main className="min-h-screen bg-[#0a0a0a] text-[#e5e5e5]" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
      ...
      <section data-sid="variations" className="py-24 px-6">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('variations') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-12 leading-relaxed">Every region, every season, every family has its own couscous. The Amazigh foundation adapts to local identity while maintaining its communal core.</p>

          <div className="flex flex-wrap gap-2 mb-8">
            {VARIATIONS.map((v, i) => (
              <button key={i} onClick={() => setActiveVariation(i)}
                className={`px-3 py-2 text-xs border transition-all duration-300 ${activeVariation === i ? 'border-[#D97706]/60 bg-[#D97706]/5 text-neutral-200' : 'border-neutral-800 text-neutral-500 hover:border-neutral-700'}`}>
                {v.name}
              </button>
            ))}
          </div>
          ...
          <div className="border border-neutral-800 p-6 md:p-8">
            <div className="flex flex-col md:flex-row md:items-start gap-6">
              <div className="flex-1">
                ...
                <p className="text-sm text-neutral-300 leading-relaxed">{VARIATIONS[activeVariation].detail}</p>
              </div>
              <div className="md:w-56 flex-shrink-0">
                <div className="border border-neutral-800 p-4 bg-[#D97706]/[0.03]">
      ...
      {/* TECHNIQUE */}
      <section data-sid="technique" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('technique') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-12 leading-relaxed">Proper couscous is never instant. The grain is steamed, broken, oiled, steamed again, broken again, buttered. Each grain separate, tender, light.</p>

          <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-8">
            {TECHNIQUE.map((t, i) => (
              <button key={i} onClick={() => setActiveStep(i)}
                className={`p-3 text-center border transition-all duration-300 ${activeStep === i ? 'border-[#D97706]/60 bg-[#D97706]/5' : 'border-neutral-800 hover:border-neutral-700'}`}>
          ...
          <div className="border border-neutral-800 p-6 md:p-8">
            ...
            <p className="text-sm text-neutral-300 leading-relaxed max-w-3xl">{TECHNIQUE[activeStep].detail}</p>
          </div>
        </div>
      </section>

      {/* FRIDAY RITUAL */}
      <section data-sid="ritual" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('ritual') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-12 leading-relaxed">Yawm al-Jumu&#x27;ah. The holiest day of the week. Schools close. Streets empty for prayer. The country gathers around one dish.</p>
            ...
            <div className="space-y-6">
              {FRIDAY_RITUAL.map((r, i) => (
                <div key={i} className="relative pl-10 md:pl-14">
                  <div className="absolute left-[11px] md:left-[19px] top-[6px] w-2.5 h-2.5 rounded-full border-2 border-[#D97706] bg-[#0a0a0a]" />
                  <div className="text-xs tracking-[0.2em] uppercase mb-1" style={{ color: ACCENT }}>{r.moment}</div>
                  <p className="text-sm text-neutral-400 leading-relaxed max-w-xl">{r.detail}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* LIFE EVENTS */}
      <section data-sid="life" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('life') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-12 leading-relaxed">Couscous marks every threshold of Moroccan life. It is never absent from the moments that matter.</p>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {LIFE_EVENTS.map((e, i) => (
              <div key={i} className="border border-neutral-800 p-5 hover:border-neutral-700 transition-colors cursor-pointer"
                onClick={() => setExpandedEvent(expandedEvent === i ? null : i)}>
                <div className="text-sm font-medium mb-2" style={{ fontFamily: "'Instrument Serif', Georgia, serif" }}>{e.occasion}</div>
                <p className={`text-xs text-neutral-400 leading-relaxed transition-all ${expandedEvent === i ? '' : 'line-clamp-2'}`}>{e.role}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ACROSS THE MAGHREB */}
      <section data-sid="maghreb" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('maghreb') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-12 leading-relaxed">
          ...
          <div className="space-y-3">
            {MAGHREB_STYLES.map((s, i) => (
              <div key={i} className="border border-neutral-800 p-5 flex flex-col md:flex-row md:items-start gap-3">
                <div className="md:w-32 flex-shrink-0">
                  <span className="text-sm font-medium" style={{ fontFamily: "'Instrument Serif', Georgia, serif" }}>{s.country}</span>
                </div>
                <p className="text-sm text-neutral-400 leading-relaxed">{s.character}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* VOCABULARY */}
      <section data-sid="vocab" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${vis('vocab') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {VOCABULARY.map((v, i) => (
              <div key={i} className="border border-neutral-800 p-4 hover:border-neutral-700 transition-colors">
                <div className="flex items-baseline gap-3 mb-1">
                  <span className="text-sm font-medium">{v.term}</span>
                  <span className="text-xs text-neutral-600 font-arabic" dir="rtl">{v.arabic}</span>
                </div>
                <p className="text-xs text-neutral-400 leading-relaxed">{v.meaning}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <footer className="py-16 px-6 border-t border-neutral-900">
        <div className="max-w-6xl mx-auto text-center">
          <p className="text-xs text-neutral-600 tracking-[0.15em] uppercase">Module 080 &#183; Couscous Friday &#183; &#169; Dancing with Lions</p>
          <p className="text-[10px] text-neutral-700 mt-2">Data: UNESCO Decision 15.COM/8.b.14, Taste of Maroc, NPR, Morocco World News</p>
        </div>
      </footer>
    </main>
  )

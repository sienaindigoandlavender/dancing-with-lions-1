...
export default function GnawaAtlasPage() {
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-[#e5e5e5]" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
        ...
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${s('instruments') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="flex gap-2 mb-8">
            {INSTRUMENTS.map((inst, i) => (
              <button key={i} onClick={() => setActiveInstrument(i)}
                className={`flex-1 text-left p-4 border transition-all duration-300 ${activeInstrument === i ? 'border-[#8B2FC9]/60 bg-[#8B2FC9]/5' : 'border-neutral-800 hover:border-neutral-700'}`}>
          ...
          <div className="border border-neutral-800 p-6 md:p-8">
            ...
            <div className="space-y-4 text-sm text-neutral-400 leading-relaxed">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3 border-t border-neutral-800/50">
              ...
              <div className="pt-3 border-t border-neutral-800/50">
                ...
                <p className="text-neutral-300">{INSTRUMENTS[activeInstrument].ancestor}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ── THE SEVEN COLOURS ── */}
      <section data-sid="colours" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${s('colours') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="flex gap-1 mb-8">
            {SEVEN_COLOURS.map((c, i) => (
              <button key={i} onClick={() => setActiveColour(i)}
                className={`flex-1 h-14 relative transition-all duration-300 border ${activeColour === i ? 'border-neutral-400 scale-105 z-10' : 'border-transparent hover:border-neutral-700'}`}
          ...
          <div className="border border-neutral-800 p-6 md:p-8" style={{ borderLeftColor: SEVEN_COLOURS[activeColour].hex === '#F5F5F5' ? '#ccc' : SEVEN_COLOURS[activeColour].hex, borderLeftWidth: '3px' }}>
            <div className="flex items-baseline gap-3 mb-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-neutral-400 leading-relaxed">
      ...
      {/* ── THE LILA ── */}
      <section data-sid="lila" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${s('lila') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="flex gap-2 mb-8">
            {LILA_PHASES.map((p, i) => (
              <button key={i} onClick={() => setActivePhase(i)}
                className={`flex-1 text-left p-3 border transition-all duration-300 ${activePhase === i ? 'border-[#8B2FC9]/60 bg-[#8B2FC9]/5' : 'border-neutral-800 hover:border-neutral-700'}`}>
          ...
          <div className="border border-neutral-800 p-6 md:p-8">
            ...
            <p className="text-sm text-neutral-400 leading-relaxed mb-3">{LILA_PHASES[activePhase].description}</p>
      ...
      {/* ── THE MAALEMS ── */}
      <section data-sid="maalems" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${s('maalems') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-8">
            {visibleMaalems.map((m, i) => (
              <button key={i} onClick={() => setActiveMaalem(i)}
                className={`text-left p-3 border transition-all duration-300 ${activeMaalem === i ? 'border-[#8B2FC9]/60 bg-[#8B2FC9]/5' : 'border-neutral-800 hover:border-neutral-700'}`}>
          ...
          {!showAllMaalems && MAALEMS.length > 4 && (
            <button onClick={() => setShowAllMaalems(true)}
              className="mb-8 text-xs tracking-[0.15em] uppercase hover:text-neutral-300 transition-colors" style={{ color: ACCENT }}>
          ...
          {activeMaalem < visibleMaalems.length && (
            <div className="border border-neutral-800 p-6 md:p-8">
              ...
              <div className="space-y-3 text-sm text-neutral-400 leading-relaxed">
                ...
                <div><span className="text-neutral-500 text-xs uppercase tracking-wider block mb-1">Significance</span><span className="text-neutral-300">{MAALEMS[activeMaalem].significance}</span></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-3 border-t border-neutral-800/50">
      ...
      {/* ── REGIONAL STYLES ── */}
      <section data-sid="styles" className="py-24 px-6 border-t border-neutral-900">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {REGIONAL_STYLES.map((r, i) => (
              <div key={i} className="border border-neutral-800 p-5 hover:border-neutral-700 transition-colors">
                ...
                <p className="text-xs text-neutral-400 leading-relaxed">{r.character}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {KEY_VOCABULARY.map((v, i) => (
              <button key={i} onClick={() => setExpandedVocab(expandedVocab === i ? null : i)}
                className={`w-full text-left border transition-all duration-300 ${expandedVocab === i ? 'border-neutral-700 bg-white/[0.01]' : 'border-neutral-800 hover:border-neutral-700'}`}>
                ...
                {expandedVocab === i && (
                  <div className="px-4 pb-4">
                    <p className="text-sm text-neutral-400 leading-relaxed">{v.definition}</p>
                  </div>
                )}
              </button>
            ))}
      ...
      <section data-sid="timeline" className="py-24 px-6 border-t border-neutral-900">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${s('timeline') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-sm text-neutral-400 max-w-2xl mb-8 leading-relaxed">

...
export default function SardineCurrent() {
  ...
  useEffect(() => {
    ...
    import('mapbox-gl').then((mapboxgl) => {
      ...
      const map = new mapboxgl.default.Map({
        ...
        center: [-14, 28], zoom: 3.8, minZoom: 2.5, maxZoom: 10,
      ...
      map.on('load', () => {
        ...
        map.addSource('current-path', { type: 'geojson', data: { type: 'Feature', properties: {}, geometry: { type: 'LineString', coordinates: coords } } })
        ...
        CURRENT_PATH.forEach((p) => {
          ...
          const popup = new mapboxgl.default.Popup({ offset: 10, closeButton: false, maxWidth: '260px' })
            .setHTML('<div style="font-family:IBM Plex Mono,monospace;padding:2px 0"><p style="font-weight:700;font-size:12px;color:#E0F0FF;margin:0 0 4px">' + p.label + '</p><p style="font-size:11px;color:#ccc;line-height:1.5;margin:0">' + p.detail + '</p></div>')
        ...
        PORTS.forEach((p) => {
          ...
          const popup = new mapboxgl.default.Popup({ offset: 8, closeButton: false, maxWidth: '250px' })
            .setHTML('<div style="font-family:IBM Plex Mono,monospace;padding:2px 0"><p style="font-size:10px;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:' + countryColor + ';margin:0 0 2px">' + p.country + '</p><p style="font-weight:700;font-size:12px;color:#fff;margin:0 0 4px">' + p.name + '</p><p style="font-size:11px;color:#ccc;margin:0">' + p.tonnes + ' tonnes &middot; ' + p.type + '</p></div>')
          new mapboxgl.default.Marker({ element: el }).setLngLat([p.lng, p.lat]).setPopup(popup).addTo(map)
        })
    ...
    <div style={{ background: C.bg, color: C.ink, fontFamily: "var(--font-plex-mono), 'IBM Plex Mono', monospace" }}>
      ...
      <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: 'clamp(80px, 15vh, 160px) clamp(24px, 5vw, 64px) clamp(40px, 8vh, 80px)' }}>
        <div ref={hero.ref} style={{ maxWidth: 900, opacity: hero.visible ? 1 : 0, transform: hero.visible ? 'none' : 'translateY(30px)', transition: 'all 1.2s cubic-bezier(0.23, 1, 0.32, 1)' }}>
          <p style={{ fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: C.muted, marginBottom: 16, fontWeight: 500 }}>Module 116 &middot; Ocean Systems</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 40 }}>
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 180 }}>
                {STOCK_DATA.map((d, i) => (<div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}><span style={{ fontSize: 10, color: C.mid }}>{d.portugal}</span><div style={{ width: '100%', height: stockSection.visible ? (d.portugal / 120) * 160 : 0, maxWidth: 40, background: d.portugal < 30 ? C.portugal : C.portugal + '88', borderRadius: '2px 2px 0 0', transition: 'height 0.8s cubic-bezier(0.23,1,0.32,1) ' + (i * 60) + 'ms' }} /><span style={{ fontSize: 9, color: C.light }}>{d.year}</span></div>))}
              </div>
            </div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: C.morocco, marginBottom: 16 }}>Morocco (national landings, K tonnes)</div>
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 180 }}>
                {STOCK_DATA.map((d, i) => (<div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}><span style={{ fontSize: 10, color: C.mid }}>{d.morocco}</span><div style={{ width: '100%', height: stockSection.visible ? (d.morocco / 1000) * 160 : 0, maxWidth: 40, background: d.morocco < 600 ? '#B45309' : C.morocco, borderRadius: '2px 2px 0 0', transition: 'height 0.8s cubic-bezier(0.23,1,0.32,1) ' + (i * 60) + 'ms' }} /><span style={{ fontSize: 9, color: C.light }}>{d.year}</span></div>))}
              </div>
            </div>
          </div>
      ...
      <section ref={fourSystems.ref} style={{ padding: '80px 24px', background: C.oceanDark }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20 }}>
            {FOUR_SYSTEMS.map((s, i) => { const isC = s.name === 'Canary'; return (
      ...
      <section style={{ padding: '80px 24px', background: C.bg }}>
        <div style={{ maxWidth: 680, margin: '0 auto' }}>
          ...
          {BIBLIOGRAPHY.map((b, i) => (<p key={i} style={{ fontSize: 12, lineHeight: 1.8, color: C.mid, marginBottom: 8, paddingLeft: 24, textIndent: -24 }}>{b}</p>))}
        </div>
      </section>

      <footer style={{ background: 'linear-gradient(to bottom, ' + C.dark + ', ' + C.darkMid + ', ' + C.darkEnd + ')', padding: '40px 24px', textAlign: 'center' }}>
        <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.06em' }}>&copy; {new Date().getFullYear()} Dancing with Lions &middot; J. Ng &middot; Ocean Systems</div>
      </footer>
    </div>
  )
}
